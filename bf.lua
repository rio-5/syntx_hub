...
local A =
    z:MakeWindow(
    {
        Name = "SYNTX ( created by james lim!! )",
        IntroEnabled = false,
        IntroText = "SYNTX Library",
...
local bI = C:AddLabel("player playing count: " .. PlayersCount() .. "/" .. game.Players.MaxPlayers)
local bJ = C:AddLabel("server time: " .. function6() .. " | " .. function7())
local bK = C:AddLabel("your aura status: " .. tostring(PlayerAura_Status))
local bL = C:AddLabel("ancient one status: " .. tostring(CheckAcientOneStatus()))
task.spawn(
    function()
        while task.wait() do
            pcall(
                function()
                    bI:Set("player playing count: " .. PlayersCount() .. "/" .. game.Players.MaxPlayers)
                end
            )
            pcall(
                function()
            ...
            pcall(
                function()
                    bK:Set("your aura status: " .. tostring(PlayerAura_Status))
                end
            )
            pcall(
                function()
                ...
                if k ~= game.JobId and v["Count"] < bO then
                    if not bM[k] or tick() - bM[k].Time > 60 * 10 then
                        ...
                        if game:GetService("Players").LocalPlayer.PlayerGui.Main.InCombat.Visible then
                            ...
                            Notify("SCRIPT STATUS:", "joining server...\nserver id: " .. k .. "\nregion: " .. v["Region"], 15)
                        else
                            Notify("SCRIPT STATUS:", "joining server...\nserver id: " .. k .. "\nregion: " .. v["Region"], 15)
                        end
...
MethodFarm_Dropdown =
    D:AddDropdown(
    {Name = "choose method farm", Default = h["MethodFarm"], Options = MethodFarms, Callback = function(bm)
